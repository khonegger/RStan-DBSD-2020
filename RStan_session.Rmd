...
Finally, we will build a multilevel (hierarchical) model that lives somewhere between the fully pooled model and the unpooled model. The great thing about multilevel models is that they give us the ability to "partially" pool the data - meaning that we allow the predictor effects to vary by county, but those county-level effects "share" strength and are not totally independent, like they are in the unpooled model. This means that counties with very few houses will benefit by having their estimates anchored by more highly sampled counties.

$$y_{ij} \sim \mathcal{N}(\alpha_{j} + \beta x_{ij}, \sigma_y), \\
\alpha_j \sim \mathcal{N}(\gamma_0 + \gamma_1u_j, \sigma_{\alpha})$$

As before, $y_{ij}$ is the log(radon) level of house $i$ in county $j$, $x_{ij}$ is whether a measurement was taken on the first floor ($X = 1$) or basement ($X = 0$), and $u_j$ is the log(uranium) in county $j$. Notice that we now have two $\sigma$'s. As before, $\sigma_y$ represents the (within-county) variation otherwise unaccounted for, but $\sigma_{\alpha}$ is interesting. It represents the variation between counties that isn't accounted for by uranium levels. Again we are fitting a separate intercept, $\alpha_j$, for each county, $j$, but now $\alpha_j$ is itself a linear function of the county uranium level. As before, $\beta$ is the effect of having a basement, and now $\gamma_1$ is the contribution of soil uranium level to county-level offsets. These effects do not differ by county.

Let's update our Stan model specification to reflect these changes.


# Stan/Bayesian modeling resources
